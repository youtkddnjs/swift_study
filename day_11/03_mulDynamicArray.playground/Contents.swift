...
let src = [ [100], [80, 70],   [50,40,30]]

let order = ["아시아노", "아시아노","아메리카노", "아프리카노","아메리카노", "아시아노"  ]

var reSrc = Array(repeating: 0, count: src.count)

for oo in order{
    //var ss = src[title.firstIndex(of: oo)!]
    //print(oo, title.firstIndex(of: oo)!,ss, ss.count)
    
    //for i in 0..<ss.count{
    for i in 0..<src[title.firstIndex(of: oo)!].count{
        reSrc[i] += src[title.firstIndex(of: oo)!][i]
    }
